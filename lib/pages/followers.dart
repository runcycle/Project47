...
class _FollowersState extends State<Followers>
  ...
  getFollowing() async {
    ...
    QuerySnapshot snapshot =
        await followersRef.doc(currentUserId).collection("userFollowers").get();
    followersIds = snapshot.docs.map((doc) => doc.id).toList();
    print(followersIds);
    setState(() {
      followersIds.forEach((doc) {
        Future<QuerySnapshot> users =
            usersRef.where("id", isLessThanOrEqualTo: doc).get();
        followersFuture = users;
      });
    });
  }
    ...
    return FutureBuilder(
        ...
        builder: (context, snapshot) {
          ...
          snapshot.data.docs.forEach((doc) {
            ...
            UserResult followersResult = UserResult(user);
            followersResults.add(followersResult);
          });
          return ListView(
    ...
    return Container(
      child: Center(
        child: ListView(
          shrinkWrap: true,
          children: <Widget>[
            Center(child: Text("There are no users following you", style: TextStyle(fontSize: 25, fontWeight: FontWeight.bold)))
          ],
        ),
      ),
    );
